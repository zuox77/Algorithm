...
2. 思路为：
    ...
    2. 如果当前是高度在持续下降，那么保存所有的下标，又由于单调栈可以保证顺序（其实这道题并没有用上，这题存储的是下标，只是利用了FILO特性）
        所以栈顶是当前的最小高度（同样的，因为存储的是下标，所以其实我们没有用到"单调"这个特性，而是通过条件判断，让它强行"单调"）
        例如[6,4,2,0,1,3,5]，单调栈会一直遍历，保存[6,4,2,0]（保存的是下标，不是高度本身），直到1的时候，由于1大于栈顶（0），所以0出栈，
        开始计算0的水量（因为保存的是下标，所以可以计算宽度）
