...
2. function:
    ...
    2. 当s的第i个字符和p的第j个字符, 只能通过匹配'*'上时, 即第j个字符一定是'*'时, p.charAt(j - 1) == '*'
        ...
        2. 匹配1个或者多个: dp[i][j] = dp[i - 1][j]
            p的第j个字符(下标j - 1)和第j - 1个字符(下标j - 2), 刚好能匹配上s的第i个字符(下标i - 1)时  <----  这是if statement
            由于我们规定了此时是匹配1个或者多个，所以说如果上面的条件是true
            那么此时dp[i][j] = dp[i - 1][j] -> dp[i - 1][j]代表第i - 1个是否能匹配上

            举例：s = 'baaaa', p = 'ba*'
            假设此时i = 5,j = 3，我们想判断dp[5][3] = ?
            j = 3，p的第3个字符(下标2)和第2个字符(下标1) -> "a*"
            i = 5，s的第5个字符(下标4) -> "a"（这是第四个a）
            所以上面的if statement是true，此时dp[i][j] = dp[i - 1][j]，即dp[4][3]
            dp[i - 1][j] -> dp[4][3] -> s的第4个字符(下标3，即第三个a)是否能和p的第3个字符(下标2)匹配上
            而因为j = 3的时候是'*'，所以dp[4][3]在之前的循环中，也经历过类似的计算，即dp[4][3] = dp[3][3]
            同理，dp[3][3]，s的第3个字符(下标2，即第二个a)是否能和p的第3个字符(下标2)匹配上，所以dp[3][3] = dp[2][3]
            dp[2][3]，s的第2个字符(下标1，即第一个a)是否能和p的第3个字符(下标2)匹配上，所以dp[2][3] = dp[1][3]
            dp[1][3]，s的第1个字符(下标0，即b)是否能和p的第3个字符(下标2)匹配上，此时显然不行了，
            所以此时需要用另一个逻辑（即第一种情况）：dp[i][j] = dp[i][j - 2] --> dp[1][3] = dp[1][1]
            s的第1个字符(下标0，即b)是否能和p的第1个字符(下标0)匹配上？可以
            所以 dp[1][3] = dp[2][3] = dp[3][3] = dp[4][3] = dp[5][3] = true
