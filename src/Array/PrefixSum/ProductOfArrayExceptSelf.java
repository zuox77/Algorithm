...
3. 因为首位为1，所以第二位其实才是第一位的前缀积，即错开一位计算，这样刚好可以保证前缀积和后缀积的乘积就是所需的答案
     ...
     2. 之后再创建一个数组，记录从右到左的乘积，但是也只记录[n - 1, 1]
        这样的话，两个数组相乘就是答案
     3. 答案应该为：[nums[1]*nums[2]*...*nums[n-1], nums[0]*nums[2]*...*nums[n-1], ... , nums[0]*nums[1]*...*nums[n-2]]
        即，每个位置上的数，都是除了自己以外，其他数的乘积，比如第0个数为nums[1]*nums[2]*...*nums[n-1]，第一个数为：nums[0]*nums[2]*...*nums[n-1]
     4. 按照上面说的方法去记录
        ...
        两两相乘，刚好就是答案，比如i=1时，为：(1*nums[0]) x (nums[2]*...*nums[n-1]*1)，刚好跳过nums[1]，就是答案nums[0]*nums[2]*...*nums[n-1]
     5. 举个例子[5, 2, 7, 9]
