...
2. 此题要求最高频率，所以默认堆方法中比较数值就不适用了，需要额外用一个HashMap记录频率，并重写最小堆的比较函数(即compare()函数)
3. 遍历一遍数组，算出频率
4. 遍历一遍HashMap，一个个加进堆，当堆中元素数量大于等于k时，判断当前元素与堆顶的关系，如果比堆顶大，则堆顶出，当前元素进
5. for循环一次，次数为k，将堆顶poll出，加入result
时间复杂度：O(NlogK)，N是数组长度，K是topK的K。遍历一边数组花费O(N)，将HashMap放入长度为K的最小堆里，每次加入里面排序需要logK的时间，
